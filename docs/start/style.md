...
本项目的风格指南主要是参照 `vue` 官方的[风格指南](https://cn.vuejs.org/v2/style-guide/index.html)。在真正开始使用该项目之前建议先阅读一遍指南，这能帮助让你写出更规范和统一的代码。当然每个团队都会有所区别。其中大部分规则也都配置在了[eslint-plugin-san](https://ecomfe.github.io/eslint-plugin-san/rules/)之中，当没有遵循规则的时候会报错，详细内容见[eslint](./eslint.md)章节。
...
- `@/components/BackToTop/index.san`
- `@/components/Charts/Line.san`
- `@/views/example/components/Button.san`
...
在`pages`文件下，代表路由的`.san`文件都使用横线连接 (kebab-case)，代表路由的文件夹也是使用同样的规则。
...
- `pages`下的`.san`文件代表的是一个路由，所以它需要和`component`进行区分(component 都是大写开头)
- 页面的`url` 也都是横线连接的，比如`https://www.xxx.admin/export-excel`，所以路由对应的`page`应该要保持统一
- 没有大小写敏感问题
